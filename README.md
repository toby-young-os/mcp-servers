...
- **Autonomous Server (Category 4)** – Runs reasoning internally with OpenAI and returns `reasoning_steps` plus the final answer (requires `OPENAI_API_KEY`). Example: `fastmcp-math-chat --server autonomous --no-planner --show-json` and ask “What is the result of doubling 7 and then subtracting 3?”
